...
IntegrationFunction::MakeIntegrationFunctionWithParamTuples(
  ...
  static long int integration_functions_count = 0;
...
IntegrationFunction::GetNodesMappedToNode(const Node* map_target) {
...
absl::StatusOr<std::vector<Node*>> IntegrationFunction::GetOperandMappings(const Node* original) {
  XLS_RET_CHECK(!IntegrationFunctionOwnsNode(original));
...
bool IntegrationFunction::IsMappingTarget(const Node* node) {
